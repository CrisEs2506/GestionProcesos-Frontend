export default function RR(quantum, arrayProcesos) {
    ...
    while (!fin) {
      ...
      if (finalPriority) {
        ...
        const indiceAntesDe = arrayProcesos.findIndex(
          (objeto) => objeto.programa.llegada > finalTick
        );
        ...
        if (indiceAntesDe !== -1) {
          ...
          const objetosAMover = arrayProcesos.filter((objeto) =>
            finalPriorityPID.includes(objeto.programa.pid)
          );
          arrayProcesos = arrayProcesos.filter(
            (objeto) => !finalPriorityPID.includes(objeto.programa.pid)
          );
          ...
          // Filtrar el array para obtener los objetos que deseas mover
          let objetosAMover = arrayProcesos.filter((objeto) =>
            finalPriorityPID.includes(objeto.programa.pid)
          );
          objetosAMover = orderArrayProcesos(objetosAMover);
          console.log(objetosAMover);
          // Filtrar el array para excluir los objetos que deseas mover
          const arraySinObjetosAMover = arrayProcesos.filter(
            (objeto) => !finalPriorityPID.includes(objeto.programa.pid)
          );
  
          // Concatenar los objetos movidos al final del nuevo array
          arrayProcesos = arraySinObjetosAMover.concat(objetosAMover);
        }
      }
      plan++;
